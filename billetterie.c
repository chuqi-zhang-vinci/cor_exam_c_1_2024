...
int main(int argc, char const *argv[])
{
  ...
  char userid[MAX_LENGTH+2]; // +2 pour \n et \0
  char from[MAX_LENGTH+2];
  char to[MAX_LENGTH+2];
  char date[MAX_LENGTH+2];
  ...
  while (user_choice != 0) {
    ...
    case 1 :
      ...
      if(!get_data_from_user(to, MAX_LENGTH+2)){
        printf("ERROR reading to");
        exit(1);
      }
      printf("What is the date ?\n");
      // TODO
      if(!get_data_from_user(date, MAX_LENGTH+2)){
    ...
    case 2:
      ...
      // TODO
      int index = find_ticket(db, nbr_tickets, userID, date);
...
/**
 * PRE: buffer is a pointer to a string of size sz
 * POST: reads at most sz characters from stdin, stores them into buffer,
...
char** create_ticket(char* userID, char* from, char* to, char* date) {
  ...
  // TODO
  char** ticket = malloc (NBR_INFOS * sizeof(char*));
  if(ticket == NULL) return NULL;

  ticket[USERID_IDX] = malloc((strlen(userID)+1) * sizeof(char));
  if(ticket[USERID_IDX] == NULL) return NULL;

  ticket[FROM_IDX] = malloc((strlen(from)+1) * sizeof(char));
  if(ticket[USERID_IDX] == NULL) return NULL;

  ticket[TO_IDX] = malloc((strlen(to)+1) * sizeof(char));
  if(ticket[USERID_IDX] == NULL) return NULL;

  ticket[DATE_IDX] = malloc((strlen(date)+1) * sizeof(char));
  if(ticket[USERID_IDX] == NULL) return NULL;
...
bool update_ticket(char** ticket, char* from, char* to) {
  // TODO
  if(strlen(from) > 0){
    ticket[FROM_IDX] = realloc(ticket[FROM_IDX], (strlen(from)+1)*sizeof(char));
    if(ticket[FROM_IDX] == NULL) return NULL;

    strcpy(ticket[FROM_IDX], from);
  }

  if(strlen(to) > 0){
    ticket[TO_IDX] = realloc(ticket[TO_IDX], (strlen(to)+1)*sizeof(char));
    if(ticket[TO_IDX] == NULL) return NULL;
